  ...
  <div class="page-wrap"><div class="inner-wrap">
	...
	<div class="cate-tab-area">
	  ...
	  <div class="tabContent clearfix">
	       <ul id="tabClassify" class="clearfix tab-item-ul">
	      	<c:forEach items="${storeData.menu}" var="menuItem">
	      		<li class="fl">
		          <a href="javascript:;" class="type" title="${menuItem.ftagcontent }">
		            <span class="name">${menuItem.ftagcontent }</span>
		          </a>
		        </li> 
			</c:forEach>
	        <li class="active fl">
 	...
 	<div class="food-nav">
		...
		<div class="category">
			<h3 class="title title-0" title="私房特色抄手"><span class="tag-na">私房特色抄手</span></h3>
			<div class="pic-food-cont clearfix">	 
				<div class="j-pic-food pic-food" id="776508279">
					...
					<div class="np clearfix">
					  <span class="name fl" title="紫苏煎泥鳅">紫苏煎泥鳅</span>
					</div>
		...
		<c:forEach items="${storeData.menu}" var="menuItem">
					<div class="category">
						<h3 class="title title-0" title="${menuItem.ftagcontent }"><span class="tag-na">${menuItem.ftagcontent }</span></h3>
						<div class="pic-food-cont clearfix">
						
	      		<c:forEach items="${storeData.items}" var="item">	
	      			
						<c:if test="${menuItem.ftagid==item.ftagid }">
							<div class="j-pic-food pic-food">
								<div class="avatar">
								  <img src="${item.fimage }"  class="food-shape scroll-loading" alt="美团外卖-${item.fname }">
								</div>
				
								<div class="np clearfix">
								  <span class="name fl" title="${item.fname }">${item.fname }</span>
								</div>
				
								<div class="labels clearfix">
									<div id="food-${item.fid }" class="food-addcart contextCannotSelected fr">+</div>
									<span class="food-number fr" style="display:none;">0</span>
								    <div class="price fl"><div class="only">¥${item.fprice }</div></div>
								</div>
						  	</div>
						  </c:if> 
				</c:forEach>
